#!/usr/bin/env python3
"""Overlay the validation curves of every ConfRover fine-tune on one chart.
...
* **The metric's own noise.** The v888 run's val_fwd has sd 0.0095 across 27
  points at a fixed config (3.4% relative). Differences smaller than that band
...
#: Order is chronological, which is also the order the story happened in.
RUNS = [
    # NOT plotted: v888 trained --window_frames 1 (single source->target pairs),
    # a strictly easier objective than a 9-frame window, so its 0.2686 is a
    # different quantity wearing the same name. Drawing it on these axes would
    # invite exactly the cross-objective comparison the number cannot support --
    # and with a y-range set for the 9-frame runs it would sit off-canvas while
    # the legend claimed it was there. Reported in the footer instead.
    Run("v888", "v888  DPF from base, 1-frame pairs (NOT comparable)",
        REPO / "runs/dpf_base_train_v888/logs/console.log", "#8c8c8c",
        "different objective: --window_frames 1", comparable=False),
    Run("cluster", "PDBcluster_from_base  95% PDB clusters, 9-frame",
        STORE / "PDBcluster_from_base_logs/console.log", "#d1495b",
        "different corpus: static structures, no time axis"),
    Run("dpf_from_cluster", "dpf_from_PDBcluster  DPF on cluster weights",
        STORE / "dpf_from_PDBcluster/logs/console.log", "#edae49",
        "stage 2: its first 500 steps undo stage 1"),
    Run("v2", "dpf_from_base_v2  DPF from base, lr 3e-5 + EMA",
        STORE / "dpf_from_base_v2/logs/console.log", "#00798c",
        "the recipe that finally moved: warm-up 500, accum 4, EMA 0.999"),
]
...
#: sd of val_fwd over 27 points of one fixed config (v888). Anything inside
...
def plot(runs: list[Run], out: Path) -> None:
    ...
    for ax, series, title in (
        ...
        if series == "fwd":
            ...
            ax.axhline(
                BASE_FORWARD_LEVEL, color="#333333", ls="--", lw=1.4, zorder=1,
        ...
        for run in runs:
            ...
            if not xs:
                continue
            ax.plot(xs, ys, color=run.colour, lw=1.7, label=run.label, zorder=3)
    ...
    fig.suptitle(
        "ConfRover fine-tunes on held-out DPF families -- every run against the untuned base",
        fontsize=13,
    )
    fig.text(
        0.5, 0.005,
        "Shaded band = +/-1 sd of the metric's own within-run scatter (0.0095, measured over 27 points of one "
        "fixed config). A gap smaller than the band is not a result.   "
        "v888 (1-frame pairs, 0.2686) is omitted: a different objective, not a better model.",
