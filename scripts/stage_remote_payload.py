...
one chosen weights file, e.g. the original base checkpoint, under ``confrover_ckpts/``.
...
from confrover.data.dpf.split import DpfSplit, catalog_fingerprint  # noqa: E402
...
def validate_remote_root(raw: str) -> str:
    ...
    if mangled:
        raise SystemExit(
            ...
            f"or double the leading slash: --remote_root //workspace/confrover_data"
        )
    return raw.rstrip("/")
...
def stage_reprs(
    ...
    if missing:
        raise KeyError(
            ...
            f"run `confrover openfold_repr` before staging. First: "
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="One weights .pt to ship under confrover_ckpts/ (e.g. "
        "original_confrover_base_20m_v1_0.pt). Default: everything under "
        "<cache_dir>/confrover_ckpts.",
    )
    parser.add_argument(
        "--pdbc_root",
        type=Path,
    ...
    parser.add_argument(
        "--remote_root",
        default="/workspace/confrover_data",
