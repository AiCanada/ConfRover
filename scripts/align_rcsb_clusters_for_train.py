...
from confrover._ext.openfold.np import residue_constants as rc  # noqa: E402
from confrover.data.dpf.catalog import DpfCatalog, seqres_from_pdb  # noqa: E402
...
DEFAULT_SELECTION = REPO_ROOT / "confrover_cache" / "pdb_cluster_selection_95_over10.csv"
...
DEFAULT_SEQRES_CSV = REPO_ROOT / "confrover_cache" / "pdbc95_over10_seqres_index.csv"
DEFAULT_PROGRESS = REPO_ROOT / "confrover_cache" / "pdbc95_over10_align_progress.jsonl"
MAX_SEQLEN = 384
MIN_SEQLEN = 20
MIN_MEMBERS = 2
MIN_RMSD = 2.0
...
def _download_cif(entry: str, dest: Path) -> None:
    """Fetch one mmCIF without clobbering a sibling worker's download."""
    import urllib.request

    dest.parent.mkdir(parents=True, exist_ok=True)
    if dest.is_file() and dest.stat().st_size > 0:
        return
    url = f"https://files.rcsb.org/download/{entry.upper()}.cif"
    ...
    *,
    ...
    for token in members:
        ...
        try:
            ...
            continue
        if pdb in seen_entry:
            continue
        seen_entry.add(pdb)
        try:
            cif, atoms = load_cif(pdb, cache_dir)
        except Exception:
            continue
        try:
        ...
        for chain_id in chains or dict.fromkeys(str(c) for c in atoms.chain_id):
            ...
            if best is None or rmsd < best_rmsd:
                best = (f"{pdb.lower()}_{chain_id}", built)
                best_rmsd = rmsd
        if best is None:
            continue
...
def stage_msa_pairs(args: argparse.Namespace, pairs: list[tuple[str, str]]) -> None:
    from confrover.data.msa.msa_loader import MSALoader
...
def _already_embedded_seqres(folding_repr: Path) -> set[str]:
    ...
    for path in (
        ...
        REPO_ROOT / "confrover_cache" / "pdbc_seqres_index.csv",
    ):
        if not path.is_file():
            continue
...
def _ten_cap_family_ids() -> set[str]:
    ids: set[str] = set()
    sel = REPO_ROOT / "confrover_cache" / "pdb_cluster_selection.csv"
    if sel.is_file():
...
def main() -> None:
    ...
    parser.add_argument(
        "--msa_root", type=Path, default=REPO_ROOT / "confrover_cache" / "msa"
    )
    parser.add_argument(
        "--folding_repr",
        type=Path,
        default=REPO_ROOT / "confrover_cache" / "folding_repr",
    )
    parser.add_argument(
        "--openfold_params",
        type=Path,
        default=REPO_ROOT / "confrover_cache" / "openfold_params",
    )
    ...
    parser.add_argument("--max_admit", type=int, default=MAX_ADMIT)
