"""One-shot vendor of OpenFold (pl_upgrades) into confrover._ext.openfold."""
...
DST = ROOT / "src" / "confrover" / "_ext" / "openfold"
PROPS_SRC = ROOT / "src" / "confrover" / "_patch" / "openfold" / "stereo_chemical_props.txt"
...
def rewrite_source(text: str) -> str:
    def repl(match: re.Match[str]) -> str:
        ...
        return f"{indent}{kind} confrover._ext.openfold{rest}"

    text = IMPORT_RE.sub(repl, text)
    text = text.replace(
        'resources.read_text("openfold.resources"',
        'resources.read_text("confrover._ext.openfold.resources"',
    )
    return text
...
def patch_attention_import() -> None:
    ...
    text = text.replace(
        "from confrover._ext.openfold.utils.kernel.attention_core import attention_core\n",
        "",
    )
    old = "            o = attention_core(q, k, v, *((biases + [None] * 2)[:2]))"
    new = (
        "            from confrover._ext.openfold.utils.kernel.attention_core "
...
def patch_script_utils() -> None:
    ...
    text = text.replace(
        "from confrover._ext.openfold.model.model import AlphaFold\n"
        "from confrover._ext.openfold.np import residue_constants, protein\n"
        "from confrover._ext.openfold.np.relax import relax\n"
        "from confrover._ext.openfold.utils.import_weights import (\n"
        "    import_jax_weights_,\n"
...
def rewrite_confrover_imports() -> None:
    skip_roots = {DST}
    files = []
    for path in (ROOT / "src" / "confrover").rglob("*.py"):
    ...
    for path in files:
        text = path.read_text(encoding="utf-8")
        new = IMPORT_RE.sub(
            lambda m: f"{m.group('indent')}{m.group('kind')} confrover._ext.openfold{m.group('rest')}",
            text,
        )
        if new != text:
