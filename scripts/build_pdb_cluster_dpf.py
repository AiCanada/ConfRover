...
* ``--catalog_out``  a catalog JSON spanning the ATLAS DPF root and this one.
  ...
  leaves untouched.
* ``--seqres_index_out``  the ``seqres,index`` CSV that ``confrover query_msa``
  and ``confrover openfold_repr`` consume. Every new family needs a
...
from confrover._ext.openfold.np.protein import Protein, to_pdb  # noqa: E402
from confrover.data.dpf.catalog import (  # noqa: E402
    DpfCatalog,
    seqres_from_pdb,
)
...
DEFAULT_CACHE = REPO_ROOT / "confrover_cache"
