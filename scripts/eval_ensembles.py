...
The diffusion validation loss cannot resolve a ConfRover fine-tune. Measured on
...
#: Euler-SDE steps. ``ConfRover.from_pretrained`` leaves ``decoder.sampler`` at
...
#: Diagnostics ``confrover.eval.ensemble_metrics.collapse_guard`` folds into the
...
DEFAULT_CATALOG = REPO_ROOT / "confrover_cache" / "merged_catalog.json"
DEFAULT_SPLIT = REPO_ROOT / "runs" / "dpf_base_train_v888" / "splits" / "0.json"
DEFAULT_FOLDING_REPR = REPO_ROOT / "confrover_cache" / "folding_repr"
...
# SIBLING SEAM.
#
# scripts/eval_ensembles.py, confrover/eval/reference.py and
# confrover/eval/ensemble_metrics.py were written concurrently against three
...
def resolve_deps() -> EvalDeps:
    from confrover.eval.ensemble_metrics import (
        collapse_verdict,
        ensemble_metrics,
        reference_control,
    )
    from confrover.eval.reference import (
...
def generate_ensemble(
    ...
    Built straight on ``ConfRover._ar_sample`` rather than the ``generate`` CLI:
    ...
    from confrover.data.infer import GenCaseConfig, GenDataset, GenDatasetConfig
    ...
    model = ConfRover.from_pretrained(str(weights))
    model.eval()
    # from_pretrained leaves decoder.sampler at None -- only ConfRover.generate
...
def evaluate(args: argparse.Namespace, deps: EvalDeps) -> dict[str, Any]:
    ...
    report: dict[str, Any] = {
        "config": {
            ...
            # confrover.eval.ensemble_metrics.collapse_verdict.
            "collapse_verdict_source": "confrover.eval.ensemble_metrics.collapse_verdict",
        },
        "families": {},
    }
