...
# `hf upload-large-folder` put at the root of $HF_REPO (AICanada/ConfRover-PDBcluster;
...
DATA="${DATA:-/workspace/confrover_data}"        # must match --remote_root at staging time
RUN="${RUN:-/workspace/runs/$RUN_NAME}"
WEIGHTS="${WEIGHTS:-$DATA/confrover_ckpts/original_confrover_base_20m_v1_0.pt}"
