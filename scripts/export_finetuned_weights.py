...
"""Export a Lightning training checkpoint into a `confrover generate` weights file.

`confrover train` only writes `confrover_base_dpf.pt` at the *end* of `fit`, so a
...
from confrover.train_policy import assert_base_weight_family
...
def main() -> None:
    ...
    if model_cfg is None:
        raise SystemExit("ConfRoverTrain has no export_model_cfg")
