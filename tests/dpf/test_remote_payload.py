...
from confrover.data.dpf.split import catalog_fingerprint
...
@pytest.mark.parametrize(
    "mangled",
    [
        "C:/Program Files/Git/workspace/confrover_data",
        r"C:\workspace\confrover_data",
        "workspace/confrover_data",
    ],
)
...
def test_catalog_paths_are_rebased_onto_the_instance(tmp_path):
    ...
    payload = stage_remote_payload.remote_catalog_dict(
        catalog, root.resolve(), "/workspace/confrover_data"
    )
    paths = [
        value
    ...
    for path in paths:
        assert path.startswith("/workspace/confrover_data/dpf/")
...
def _train_invocations(script: str) -> list[list[str]]:
    """Every `confrover train ...` call in the bootstrap, as argv lists.
    ...
    text = script.replace("\r\n", "\n")
    out = []
    for block in re.findall(r"confrover train(.*?)(?=\n\S|\Z)", text, re.S):
...
def test_the_bootstrap_train_command_parses(tmp_path):
    ...
    """
    from confrover.cli import build_parser

    script = (REPO_ROOT / "scripts" / "vast_bootstrap.sh").read_text(encoding="utf-8")
    calls = _train_invocations(script)
    assert calls, "no `confrover train` invocation found in vast_bootstrap.sh"
...
def test_catalog_mode_stages_every_structure_by_its_own_path(tmp_path):
    ...
    payload = stage_remote_payload.remote_catalog_dict(
        catalog, None, "/workspace/confrover_data", pdbc_root=root.resolve()
    )
    for family in payload["families"]:
        for member in family["members"]:
            assert member["pdb_path"].startswith("/workspace/confrover_data/pdbc/")
...
def test_rebasing_needs_a_root(tmp_path):
    ...
    with pytest.raises(ValueError, match="at least one"):
        stage_remote_payload.remote_catalog_dict(catalog, None, "/workspace/confrover_data")
