...
"""Regression tests for ``confrover train`` orchestration (src/confrover/train.py).
...
from confrover.data.dpf.catalog import DpfCatalog, DpfFamily, DpfMember
from confrover.data.dpf.split import DpfSplit
...
def make_args(**overrides) -> argparse.Namespace:
    ...
    for key, value in overrides.items():
        if not hasattr(args, key):
            raise AssertionError(f"{key!r} is not a `confrover train` option")
...
def test_excludelist_auto_uses_cache_dir_list(tmp_path: Path):
    from confrover.train_policy import BASE_TRAINED_IDS_FILENAME
...
def test_default_count_split_has_a_val_holdout(tmp_path: Path):
    """`confrover train` with nothing but --output must still get a val loss."""
...
def test_lr_scale_warms_up_then_decays_to_floor():
    from confrover.model.train import lr_scale
...
def test_train_help_renders():
    """`confrover train --help` must not die on an unescaped % in a help string.
    ...
    touches format_help().
    """
    parser = train_cli.add_args(argparse.ArgumentParser(prog="confrover train"))
...
def test_repr_cache_size_is_exposed_and_defaults_above_a_toy_value():
    ...
    from confrover.data.dpf.dataset import DEFAULT_REPR_CACHE_SIZE
...
def test_restart_sidecar_is_written_beside_the_checkpoint(tmp_path: Path):
    from confrover.data.datamodule import RESTART_SIDECAR_NAME, dump_restart_sidecar
...
def test_forward_stride_defaults_to_the_base_models_range():
    """ConfRover-base trained on strides 1~1024 at 10 ps (arXiv:2505.17478)."""
...
def test_a_range_never_reaches_the_generation_manifest_as_a_tuple():
    """The manifest schema is an int; a tuple would break confrover generate."""
...
def test_the_dataset_carries_its_epoch_in_shared_memory():
    ...
    from confrover.data.dpf.dataset import _shared_epoch_box
...
def test_removing_a_sidecar_that_is_already_gone_is_not_an_error(tmp_path: Path):
    from confrover.data.datamodule import drop_restart_sidecar
...
def test_the_selected_metric_is_one_the_model_actually_logs():
    ...
    source = inspect.getsource(ConfRoverTrain._log_step)
...
def test_the_census_is_skipped_when_it_decides_nothing():
    ...
    from confrover.model.utils import dead_units
...
@pytest.mark.parametrize(
    "overrides,label",
    [
        ({"catalog": r"A:\x\confrover_cache\pdbc95_over10_catalog_unique.json"}, "PDB clusters"),
