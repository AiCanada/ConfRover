...
"""``ConfRoverTrain._source_position_ids``: WHERE each context token sits in time.
...
``ConfRoverTrain`` costs a measured 2.2 s of ``ConfRover.from_config`` plus the
...
from confrover.model.train import ConfRoverTrain  # noqa: E402

MODEL_CONFIG = REPO / "src" / "confrover" / "configs" / "model" / "confrover.yaml"
...
def _position_ids(
    ...
    *,
    ...
    return ConfRoverTrain._source_position_ids(trunk, batch, inputs_embeds)
...
def test_a_fused_count_that_is_not_a_multiple_of_the_batch_is_refused():
    ...
    with pytest.raises(ValueError, match="not a multiple of batch"):
        ConfRoverTrain._source_position_ids(trunk, batch, inputs_embeds)
...
def _synthetic_window(start: int, step: int, frames: int = W):
    ...
    from confrover.data.dpf.examples import TrainExample
...
def test_a_reversed_window_gets_the_same_position_ids_as_its_ascending_twin():
    ...
    from confrover.data.dpf.examples import ReversalPolicy, orient_window
...
def test_the_widest_legal_window_fits_inside_the_rope_table_with_room_to_spare():
    ...
    """
    from confrover.train import BASE_FORWARD_STRIDE_RANGE, DEFAULT_WINDOW_FRAMES
