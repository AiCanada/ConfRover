...
import confrover.train_policy as train_policy
...
def test_model_train_policy_reexports_provenance():
    """The lead wires from_base_checkpoint through confrover.model.train_policy."""
    mod = importlib.import_module("confrover.model.train_policy")
...
def test_load_id_list_csv_name_column(tmp_path: Path):
    """Layout of confrover_cache/confrover_base_atlas_train_ids.csv."""
...
def test_load_id_list_csv_chain_column(tmp_path: Path):
    """Layout of confrover_cache/newpdbidlistrain_chains.csv."""
...
def test_dpf_root_is_env_overridable(tmp_path: Path):
    ...
    Reloading ``confrover.train_policy`` rebinds ``TrainPolicyError`` (and every
    other class in it) to fresh objects while ``confrover.model.train`` and this
...
def test_train_policy_class_identity_is_stable():
    ...
    import confrover.model.train_policy as model_policy

    assert model_policy.TrainPolicyError is train_policy.TrainPolicyError
    assert (
        sys.modules["confrover.train_policy"].TrainPolicyError
...
#
# The released ConfRover checkpoints self-identify in payload["metadata"]
