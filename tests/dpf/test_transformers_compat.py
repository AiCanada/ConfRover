...
from confrover.model.temporal import llama as llama_mod  # noqa: E402
from confrover.model.temporal.llama import (  # noqa: E402
...
def _tiny_module() -> FusedLlamaPairformerModule:
    """A 2-layer / 1-pairformer version of ``configs/model/confrover.yaml``."""
...
#: ConfRover itself always generates from embeddings, which now take the
...
def test_autoregressive_decode_loop_end_to_end(tiny_module):
    """The ``confrover generate`` path: every compat helper, four decode steps.

    Mirrors ``ConfRover._ar_sample``: prepare_configs_for_generation ->
...
def test_dead_code_decision_is_documented():
    ...
    attention, it is part of the upstream ByteDance module, and it is exercised by
...
def test_sink_cache_rejected_before_any_work(tmp_path):
    ...
    cfg = Path(__file__).resolve().parents[2] / "src/confrover/configs/model/confrover.yaml"
    if not cfg.is_file():
        pytest.skip(f"model config not found: {cfg}")
    model = ConfRover.from_config(str(cfg), seed=0, kv_cache_type="sink4:1000")
...
def test_embedding_inputs_get_an_explicit_all_ones_attention_mask():
    ...
    from confrover.model.temporal import llama as llama_mod
