...
from confrover.utils.misc.pylogger import (
    ConsoleFormatter,
    SafeStreamHandler,
...
def _attach(stream: io.TextIOWrapper) -> logging.Logger:
    logger = logging.getLogger("confrover.test.cp1252")
...
def test_attach_run_file_logging_writes_debug_and_issues(tmp_path):
    ...
    try:
        ...
        logging.getLogger("confrover.test").exception("trainer.fit failed")
    ...
    from confrover.utils.misc.pylogger import _drop_run_file_handlers
...
def test_leafspec_deprecation_is_not_logged_after_hooks(tmp_path):
    ...
    with warnings.catch_warnings(record=True) as caught:
        ...
        from confrover.utils.misc.pylogger import _ignore_known_upstream_warnings
...
def test_stdlib_handler_on_cp1252_is_what_we_are_fixing():
    ...
    raw, stream = _cp1252_stream()
    logger = logging.getLogger("confrover.test.cp1252.stdlib")
    logger.handlers.clear()
    logger.propagate = False
    logger.setLevel(logging.DEBUG)
