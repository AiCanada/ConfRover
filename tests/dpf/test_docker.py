...
def test_dockerfile_wires_the_entrypoint_to_the_bootstrap():
    ...
    assert "COPY docker/entrypoint.sh /usr/local/bin/confrover-cloud" in text
    assert 'ENTRYPOINT ["confrover-cloud"]' in text
    assert "REPO=/opt/ConfRover" in text, "code must live outside the /workspace volume"
...
def test_build_context_excludes_data_runs_and_weights():
    ...
    for must in ("runs/", "runsPDB/", "confrover_cache/", "payloads/", "*.ckpt", "*.pt", "*.npy", ".git"):
...
def test_compose_gives_the_container_gpus_and_shared_memory():
    ...
    assert "confrover_ws:/workspace" in text
