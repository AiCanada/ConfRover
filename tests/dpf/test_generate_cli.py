...
"""`confrover generate` must not require the optional DeepSpeed extra.
...
from confrover import PACKAGE_ROOT, inference
...
def test_the_fallback_warnings_do_not_prescribe_an_install_that_cannot_run(caplog):
    ...
    `pip install 'confrover[deepspeed]'` fetches a wheel on almost no
    ...
    text = caplog.text
    assert "pip install 'confrover[deepspeed]'" not in text
