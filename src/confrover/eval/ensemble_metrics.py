...
Why this module exists: ConfRover fine-tunes are being compared on the
...
r = 0.48 in its own paper but 0.56 +/- 0.06 when re-run under ConfRover's
250-conformation protocol, and the numbers moved again between ConfRover v1 and
v2. Use them to check that this implementation is wired up and in the right
units. Never quote them against a ConfRover run.
...
the pipeline already speak: ``ConfRover._ar_sample`` returns ``atom37`` in
Angstrom, and ``confrover.data.io.xtc.xtc_to_atom37(..., unit="A")`` reads
