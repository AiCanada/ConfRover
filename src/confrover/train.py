...
from confrover.data.datamodule import (
    ConfRoverDataModule,
    drop_restart_sidecar,
    dump_restart_sidecar,
)
from confrover.data.dpf import DpfCatalog, DpfSplit, SplitFractions, assert_no_leakage
from confrover.data.dpf.split import identity_components
from confrover.data.dpf.dataset import (
    DEFAULT_REPR_CACHE_SIZE,
    DpfTrainDataset,
)
from confrover.data.dpf.manifest import export_heldout_manifest, write_heldout_manifest
...
from confrover.utils import attach_run_file_logging, get_pylogger, log_header
from confrover.utils.misc.cli import str2bool
from confrover.utils.torch.callbacks import MetricsHandler
from confrover.model.utils.dead_units import repair_decoder_capacity
from confrover.utils.torch.tflops import (
    PROBE_SEQLEN,
    format_tflops,
...
class StepHeartbeat(Callback):
    ...
    def on_train_batch_end(
        ...
        extras = " ".join(
            part
            for part in (task_loss_txt, term_txt, gate_txt, mix_txt, mem_txt)
            if part
        )
...
#: ConfRover-base trained on sub-trajectories with strides 1~1024 snapshots at
...
def gen_stride_in_10ps(spec: int | tuple[int, int]) -> int:
    ...
    """
    from confrover.data.dpf.examples import scalar_forward_stride
...
def stride_spec(text: str) -> int | tuple[int, int]:
    ...
    ConfRover-base saw sub-trajectories "with varying strides (1~1024 MD
...
_SILENCED_UPSTREAM_WARNINGS: tuple[tuple[str, str], ...] = (
    (
        ...
        "lightning-internal, not reachable from confrover code",
    ),
)
...
def run_train(args: argparse.Namespace) -> None:
    ...
    model = ConfRoverTrain.from_base_checkpoint(
    ...
    if graceful_stop.stop_status is not None:
        ...
        log.warning(
            ...
            "turns any checkpoint into a `confrover generate` weights file."
        )
    ...
    if trainer.is_global_zero:
        ...
        if model_cfg is None:
            raise RuntimeError(
                "ConfRoverTrain missing export_model_cfg; cannot write a "
...
def add_args(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    data.add_argument(
        ...
        "(10 ps per frame). The default 1-1024 is what ConfRover-base was "
    ...
    data.add_argument(
        ...
        "run `confrover openfold_repr` first if check_cache reports missing seqres.",
    )
    data.add_argument(
        "--max_seqlen",
        type=int,
...
def _require_cached_reprs(repr_loader, catalog, split) -> None:
    ...
    if missing:
        raise RuntimeError(
            ...
            "Generate them first with `confrover openfold_repr` "
            f"(folding_repr={repr_loader.repr_root}). If that path is not the "
            "cache you meant: the default cache root is ./confrover_cache "
...
def _package_versions() -> dict[str, str | None]:
    ...
    for name in ("confrover", "torch", "transformers", "lightning", "numpy", "mdtraj"):
        try:
...
def _val_trainer_kwargs(val_dataset: Any, args: argparse.Namespace) -> dict[str, Any]:
    ...
    ``ConfRoverTrain`` defines ``validation_step``, so Lightning would call
