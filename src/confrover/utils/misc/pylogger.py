...
#: (``confrover train > train.log 2>&1``). StreamHandler.emit then raises
...
#: Matched with re.escape as a prefix. These are not ConfRover bugs.
...
def install_debug_hooks(fault_path=None) -> None:
    ...
    def _excepthook(exc_type, exc, tb):
        logging.getLogger("confrover").error(
