...
class ConfRoverTrain(ConfRover):
    ...
    def from_base_checkpoint(
        ...
        from confrover.model.confrover import ModelRegistry

        ckpt_root = CachePaths().confrover_ckpts if ckpt_dir is None else ckpt_dir
        ...
        if "model_cfg" not in model_ckpt or "state_dict" not in model_ckpt:
            raise RuntimeError(
                ...
                "(ConfRover from_pretrained schema)."
            )
        ...
        backbone = ConfRover.from_config(
    ...
    def on_fit_start(self) -> None:
        ...
        if self.decoder.loss is None:
            raise RuntimeError(
                "ConfRoverTrain requires decoder.loss; wire ConfDiffLoss in the train config."
            )
    ...
    def _assert_trainable(self) -> None:
        ...
        if embedder is None:
            raise RuntimeError(
                ...
                "ConfRoverTrain._embedder to match, or the embedder trains with "
    ...
    def on_save_checkpoint(self, checkpoint: dict[str, Any]) -> None:
        ...
        from confrover.data.datamodule import RESTART_CHECKPOINT_KEY
    ...
    def _source_position_ids(self, batch, inputs_embeds: torch.Tensor) -> torch.Tensor:
        """RoPE ids in ConfRover 10 ps units: BEGIN=0, then k * the example's gap.

        Matches ConfRover._ar_sample / GenDataset.get_frame_idxs, where token j
