...
#: which :class:`confrover.model.temporal.kv_cache.OffloadedCache` subclasses and
    ...
    * >=5         : gone; the 4.41 semantics are re-implemented here.
    ...
    if upstream is not None:
        ...
        logger.warning_once(
            ...
            "transformers==%s; using the ConfRover back-port instead.",
            str(names),
            TRANSFORMERS_VERSION,
        )
...
class FusedLlamaPairformerModule(LlamaPreTrainedModel, GenerationMixin):
    ...
    # module (and ``ConfRover._ar_sample``) relies on. They are re-provided here
    ...
    def _get_initial_cache_position(
        ...
        ``ConfRover._ar_sample`` use; :func:`_seed_cache_position` adapts it to
    ...
    def _update_model_kwargs_for_generation(
        ...
        **kwargs,
        ...
        decode steps of :meth:`generate` / ``ConfRover._ar_sample`` with a stale,
        ...
        **kwargs,
        ...
        if "generation_config" not in _criteria_kwargs:
            raise NotImplementedError(
                ...
                "ConfRover cannot build its stopping criteria."
            )
