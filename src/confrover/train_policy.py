...
DPF_ROOT_ENV_VAR = "CONFROVER_DPF_ROOT"
...
#: Local ATLAS DPF store. Override with ``CONFROVER_DPF_ROOT``; the historical
...
#: ``confrover train`` unless the exclusion is explicitly turned off.
...
_BASE_NAME_RE = re.compile(
    r"(?:^|[-_ ])confrover[-_ ]?(?:base[-_ ]?20m|20m[-_ ]?base)"
    r"([-_ ]?v1[._-]\d+)?$"
)
...
def assert_checkpoint_provenance(
    ...
    # Every released ConfRover checkpoint self-identifies in payload["metadata"]
